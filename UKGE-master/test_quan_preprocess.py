...
pattern = re.compile(r"^Predicted_score_(\w+)_ModelList\.(\w+)_(\d+)_([\d\.]+)_\d+\.csv$")


csv_files = glob.glob("PS/*.csv")
...
for file in csv_files:
    ...
    if m:
        dataset, model, seed, quantile = m.groups()
...
for (dataset, model, seed), files in groups.items():
    if "0.05" in files and "0.95" in files:
        ...
        labels = df_lower["w"]
        lower_scores = df_lower["scores"]
        upper_scores = df_upper["scores"]
        ...
        if key not in results:
            results[key] = []
        results[key].append(new_row)

for (dataset, model), rows in results.items():
    df_result = pd.DataFrame(rows)
    output_filename = f"quan_{model}_{dataset}.csv"
