...
class Tester(object):
    ...
    def get_mse(self, save_dir='', epoch=0, is_test=False):
        def calculate_metrics(test_triples, qhat):
            ...
            pred_intervals = [[max(0, s - qhat), min(1, s + qhat)] for s in scores]
            ...
            return coverage, sharpness, scores, w_batch

        def normalizing_calculate_metrics(test_triples, qhat):
            h_batch = test_triples[:, 0].astype(int)
            r_batch = test_triples[:, 1].astype(int)
            t_batch = test_triples[:, 2].astype(int)
            w_batch = test_triples[:, 3]
            ...
            scores = self.get_score_batch(h_batch, r_batch, t_batch)
            p_test = np.clip(scores, 1e-6, 1 - 1e-6)
            entropy_test = - (p_test * np.log(p_test) + (1 - p_test) * np.log(1 - p_test))
            # calculate the uncertainty
            uncertainty_test = np.clip(entropy_test, 1e-6, None)
            ...
            return coverage, sharpness, scores, w_batch, mean_pred_intervals
        
        def normalizing_calculate_metrics_mapping(test_triples, qhat):
            h_batch = test_triples[:, 0].astype(int)
            r_batch = test_triples[:, 1].astype(int)
            t_batch = test_triples[:, 2].astype(int)
            w_batch = test_triples[:, 3]
            ...
            scores = self.get_score_batch(h_batch, r_batch, t_batch)
            scores_mapping = 0.5 * scores + 0.5
            p_test = np.clip(scores_mapping, 1e-6, 1 - 1e-6)
            entropy_test = - (p_test * np.log(p_test) + (1 - p_test) * np.log(1 - p_test))
            # calculate the uncertainty
            uncertainty_test = np.clip(entropy_test, 1e-6, None)
        ...
        if not is_test:
            ...
            scores = self.get_score_batch(
                test_triples[:, 0].astype(int),
                test_triples[:, 1].astype(int),
                test_triples[:, 2].astype(int)
            )

            errors = scores - w_batch
            mse = np.mean(np.square(errors))
            mae = np.mean(np.abs(errors))

            param.valid_scores = scores
            param.valid_errors = errors
            nonconf_scores = np.abs(errors)
            conf_level = np.ceil((len(nonconf_scores) + 1) * 0.9) / len(nonconf_scores)
            param.qhat = np.quantile(nonconf_scores, conf_level, interpolation="higher")
            param.errors = errors
            param.valid_pos_scores = scores
            param.valid_pos_label = w_batch
            
            # nonconformal score for normalizing conformal prediction
            p_pos = np.clip(scores, 1e-6, 1 - 1e-6)
            entropy = -p_pos * np.log(p_pos) - (1 - p_pos) * np.log(1 - p_pos)
            uncertainty = np.clip(entropy, 1e-6, None)
            
            # nonconformal score for normalizing conformal prediction
            normalized_nonconf_scores = np.abs(errors) / uncertainty
            norm_conf_level = np.ceil((len(normalized_nonconf_scores) + 1) * 0.9) / len(normalized_nonconf_scores)
            param.normalizing_qhat = np.quantile(normalized_nonconf_scores, norm_conf_level, interpolation="higher")
            param.normalizing_nonconf_scores = normalized_nonconf_scores
            
            # nonconformal score for normalizing conformal prediction after mapping For CN15K
            # mapping the scores from [0, 1] to [0.5, 1]
            score_mapping = 0.5 * scores + 0.5
            p_pos_mapping = np.clip(score_mapping, 1e-6, 1 - 1e-6)
            entropy_mapping = -p_pos_mapping * np.log(p_pos_mapping) - (1 - p_pos_mapping) * np.log(1 - p_pos_mapping)
            uncertainty_mapping = np.clip(entropy_mapping, 1e-6, None)
            param.uncertainty = np.mean(uncertainty_mapping)
            
            # nonconformal score for normalizing conformal prediction For CN15K
            normalized_nonconf_scores_mapping = np.abs(errors) / uncertainty_mapping
            norm_conf_level_mapping = np.ceil((len(normalized_nonconf_scores_mapping) + 1) * 0.9) / len(normalized_nonconf_scores_mapping)
            param.normalizing_qhat_mapping = np.quantile(normalized_nonconf_scores_mapping, norm_conf_level_mapping, interpolation="higher")
            param.normalizing_nonconf_scores_mapping = normalized_nonconf_scores_mapping

        if is_test:
            # Positive samples
            
            # conformal prediction
            test_pos_triples = self.test_triples[self.test_triples[:, 3] != 0]
        
            # param.test_pos_w = self.test_triples[:, 3]

            # scores = self.get_score_batch(h_batch, r_batch, t_batch)
            # scores_pos = self.get_score_batch(test_pos_triples[:, 0].astype(int), test_pos_triples[:, 1].astype(int), test_pos_triples[:, 2].astype(int))
            # param.test_pos_scores = scores_pos
            
            coverage_pos, sharpness_pos, test_pos_scores, test_pos_label = calculate_metrics(test_pos_triples, param.qhat)
            #print(f'coverage_pos: {coverage_pos}')
            #print(f'sharpness_pos: {sharpness_pos}')
            #print(f'qhat: {param.qhat}')
            param.coverage_pos = coverage_pos
            param.sharpness_pos = sharpness_pos
            param.test_pos_scores = test_pos_scores
            param.test_pos_label = test_pos_label
            
            # Normalizing conformal prediction
            normalizing_coverage_pos, normalizing_sharpness_pos, normalizing_test_pos_scores, normalizing_test_pos_label, normalizing_test_pos_new_qhat = normalizing_calculate_metrics(test_pos_triples, param.normalizing_qhat)
            
            param.normalizing_coverage_pos = normalizing_coverage_pos
            param.normalizing_sharpness_pos = normalizing_sharpness_pos
            param.normalizing_test_pos_new_qhat = normalizing_test_pos_new_qhat
            
            # Normalizing conformal prediction after mapping For CN15K
            normalizing_coverage_pos_mapping, normalizing_sharpness_pos_mapping, normalizing_test_pos_scores_mapping, normalizing_test_pos_label_mapping, normalizing_test_pos_new_qhat_mapping = normalizing_calculate_metrics_mapping(test_pos_triples, param.normalizing_qhat_mapping)
            
            param.normalizing_coverage_pos_mapping = normalizing_coverage_pos_mapping
            param.normalizing_sharpness_pos_mapping = normalizing_sharpness_pos_mapping
            param.normalizing_test_pos_new_qhat_mapping = normalizing_test_pos_new_qhat_mapping

            # Negative samples
            # conformal prediction
            test_neg_triples = self.test_triples[self.test_triples[:, 3] == 0]
            # score_neg = self.get_score_batch(test_neg_triples[:, 0].astype(int), test_neg_triples[:, 1].astype(int), test_neg_triples[:, 2].astype(int))
            
            # self.test_neg_scores = score_neg
            self.test_neg_w = test_neg_triples[:, 3]
            coverage_neg, sharpness_neg, test_neg_scores, test_neg_label = calculate_metrics(test_neg_triples, param.neg_qhat)
            #print(f'coverage_neg: {coverage_neg}')
            #print(f'sharpness_neg: {sharpness_neg}')
            #print(f'neg_qhat: {param.neg_qhat}')
            param.coverage_neg = coverage_neg
            param.sharpness_neg = sharpness_neg
            param.test_neg_scores = test_neg_scores
            param.test_neg_label = test_neg_label
            
            # Normalizing conformal prediction
            normalizing_coverage_neg, normalizing_sharpness_neg, normalizing_test_neg_scores, normalizing_test_neg_label, normalizing_test_neg_new_qhat= normalizing_calculate_metrics(test_neg_triples, param.normalizing_neg_qhat)
            
            param.normalizing_coverage_neg = normalizing_coverage_neg
            param.normalizing_sharpness_neg = normalizing_sharpness_neg
            param.normalizing_test_neg_new_qhat = normalizing_test_neg_new_qhat
            
            # Normalizing conformal prediction after mapping For CN15K
            normalizing_coverage_neg_mapping, normalizing_sharpness_neg_mapping, normalizing_test_neg_scores_mapping, normalizing_test_neg_label_mapping, normalizing_test_neg_new_qhat_mapping= normalizing_calculate_metrics_mapping(test_neg_triples, param.normalizing_neg_qhat_mapping)
            
            param.normalizing_coverage_neg_mapping = normalizing_coverage_neg_mapping
            param.normalizing_sharpness_neg_mapping = normalizing_sharpness_neg_mapping
            param.normalizing_test_neg_new_qhat_mapping = normalizing_test_neg_new_qhat_mapping

            #mse, mae
            # conformal prediction for combined
            test_triples = self.test_triples
            test_scores_combined = self.get_score_batch(
                test_triples[:, 0].astype(int),
                test_triples[:, 1].astype(int),
                test_triples[:, 2].astype(int)
            )
            param.test_scores_combined = test_scores_combined
            # param.test_w_combined = test_triples[:, 3]
            coverage_combined, sharpness_combined, test_combined_scores, test_combined_label = calculate_metrics(test_triples, param.qhat_combined)
            #print(f'coverage_combined: {coverage_combined}')
            #print(f'sharpness_combined: {sharpness_combined}')
            #print(f'qhat_combined: {param.qhat_combined}')
            param.coverage_combined = coverage_combined
            param.sharpness_combined = sharpness_combined
            param.test_combined_scores = test_combined_scores
            param.test_combined_label = test_combined_label
            
            # Normalizing conformal prediction for combined
            normalizing_coverage_combined, normalizing_sharpness_combined, normalizing_test_combined_scores, normalizing_test_combined_label, normalizing_test_combined_new_qhat = normalizing_calculate_metrics(test_triples, param.normalizing_qhat_combined)
            
            param.normalizing_coverage_combined = normalizing_coverage_combined
            param.normalizing_sharpness_combined = normalizing_sharpness_combined
            param.normalizing_test_combined_new_qhat = normalizing_test_combined_new_qhat
            
            # Normalizing conformal prediction for combined after mapping For CN15K
            normalizing_coverage_combined_mapping, normalizing_sharpness_combined_mapping, normalizing_test_combined_scores_mapping, normalizing_test_combined_label_mapping, normalizing_test_combined_new_qhat_mapping = normalizing_calculate_metrics_mapping(test_triples, param.normalizing_qhat_combined_mapping)
            
            param.normalizing_coverage_combined_mapping = normalizing_coverage_combined_mapping
            param.normalizing_sharpness_combined_mapping = normalizing_sharpness_combined_mapping
            param.normalizing_test_combined_new_qhat_mapping = normalizing_test_combined_new_qhat_mapping
    ...
    def get_mse_neg(self, neg_per_positive):
        ...
        nonconf_scores_cal_hn = np.abs(scores_hn).flatten()
        nonconf_scores_cal_tn = np.abs(scores_tn).flatten()
        nonconf_scores_cal = np.ravel(np.column_stack((nonconf_scores_cal_hn, nonconf_scores_cal_tn)))
        rng = np.random.RandomState(param.seed)
        
        # randomly sample samples
        errors_neg = np.array([
            rng.choice(nonconf_scores_cal[i:i+20]) 
            for i in range(0, len(nonconf_scores_cal), 20)
        ])
        # errors_neg = nonconf_scores_cal
        #errors_neg = nonconf_scores_cal
        #param.valid_neg_label = np.zeros_like(errors_neg)
        param.errors_neg = errors_neg
        # param.valid_neg_scores = errors_neg
        n = len(errors_neg)
        conf_level = np.ceil((n+1)*(1-0.1))/n
        neg_qhat = np.quantile(errors_neg, conf_level, interpolation="higher")
        param.neg_qhat = neg_qhat

        ## calculate the normalized nonconformity score
        # calculate the uncertainty
        p_neg = np.clip(errors_neg, 1e-6, 1 - 1e-6)
        entropy_neg = - (p_neg * np.log(p_neg) + (1 - p_neg) * np.log(1 - p_neg))
        uncertainty_neg = np.clip(entropy_neg, 1e-6, None)
        
        normalizing_errors_neg = np.abs(errors_neg) / uncertainty_neg
        param.normalizing_errors_neg = normalizing_errors_neg
        param.normalizing_neg_qhat = np.quantile(normalizing_errors_neg, conf_level, interpolation="higher")
        
        ## calculate the normalized nonconformity score after mapping For CN15K
        # calculate the uncertainty
        errors_neg_mapping = 0.5*errors_neg+0.5
        p_neg_mapping = np.clip(errors_neg_mapping, 1e-6, 1 - 1e-6)
        entropy_neg_mapping = - (p_neg_mapping * np.log(p_neg_mapping) + (1 - p_neg_mapping) * np.log(1 - p_neg_mapping))
        uncertainty_neg_mapping = np.clip(entropy_neg_mapping, 1e-6, None)
        
        normalizing_errors_neg_mapping = np.abs(errors_neg) / uncertainty_neg_mapping
        param.normalizing_errors_neg_mapping = normalizing_errors_neg_mapping
        param.normalizing_neg_qhat_mapping = np.quantile(normalizing_errors_neg_mapping, conf_level, interpolation="higher")
...
class UKGE_rect_Tester(Tester):
    ...
    def get_mse(self, save_dir='', epoch=0, is_test=False):
        def calculate_metrics(test_triples, qhat):
            ...
            scores = self.bound_score(self.get_score_batch(h_batch, r_batch, t_batch))
            pred_intervals = [[max(0, s - qhat), min(1, s + qhat)] for s in scores]
        ...
        if not is_test:
            test_triples = self.test_triples
            w_batch = test_triples[:, 3]
            scores = self.bound_score(self.get_score_batch(
                test_triples[:, 0].astype(int),
                test_triples[:, 1].astype(int),
                test_triples[:, 2].astype(int)
            ))
        ...
        if is_test:
            ...
            # Normalizing conformal prediction
            normalizing_coverage_neg, normalizing_sharpness_neg, normalizing_test_neg_scores, normalizing_test_neg_label, normalizing_test_neg_new_qhat= normalizing_calculate_metrics_mapping(test_neg_triples, param.normalizing_neg_qhat)
