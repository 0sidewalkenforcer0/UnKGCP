...
def test_func(test_data, device, model, params, threshold=None, neg_mse_also=False, ndcg_also=False, save=False):
    ...
    for ids, cls in data:
        ...
        with torch.no_grad():
            ...
            if save:
                truth_saved = torch.exp(score).cpu().numpy()
                pred_saved = label.cpu().numpy()
                data = {'truth': truth_saved, 'prediction': pred_saved}
                df = pd.DataFrame(data)
                df.to_csv(f'truth_prediction_{params.data_name}_{params.seed}.csv', index=False)
                print('truth_prediction saved by trainer')
...
def train_func(train_data, train_test_data, dev_data, test_data,
    ...
    for ids, cls in data:
        ...
        if step % 20 == 0 or (batch_size > 4096 and step % 10 ==0):
            ...
            # test
            test_MSE, test_MAE, _, _ = test_func(test_data, device, model, params, ndcg_also=False, save=False)
            wandb.log({'Test MSE': test_MSE})
            wandb.log({'Test MAE': test_MAE})

            # validation
            valid_pos_mse, valid_mae, valid_neg_mse, _ = test_func(dev_data, device, model, params, neg_mse_also=True, save=False)
    ...
    for epoch in range(params.EPOCH):
        ...
        if params.early_stop == 'ndcg' and epoch % 10 == 0:
            ...
            if linear_ndcg > last_best_ndcg:
                ...
                torch.save(model, join(params.model_dir, f'{params.whichmodel}-{params.seed}.pt'))
            else:
        ...
        if params.early_stop == 'valid_mse':
            if epoch >= 1 and best_metric['valid_mse'] >= last_best_metric['valid_mse']:  # no improvement or already overfit
                ...
                last_best_epoch = epoch
                torch.save(model, join(params.model_dir, f'{params.whichmodel}-{params.seed}.pt'))
                _, _, _, _ = test_func(test_dataset, params.device, model, params, ndcg_also=False, save=True)
