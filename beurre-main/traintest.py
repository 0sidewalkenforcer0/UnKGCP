...
def test_func(test_data, device, model, params, threshold=None, neg_mse_also=False, ndcg_also=False):
    ...
    for ids, cls in data:
        ...
        with torch.no_grad():
            ...
            #seed=params.seed
            df.to_csv(f'truth_prediction_nl27k_0.csv', index=False)
            print('truth_prediction.csv saved')'''
            ...
            if not neg_mse_also:
                ...
                label_pos = label[nonzero_mask]
                score_pos = score[nonzero_mask]

                zero_mask = label == 0
                label_neg = label[zero_mask]
                score_neg = score[zero_mask]
                return mse, mae, None, ndcg, score_pos, label_pos, score_neg, label_neg
            ...
            combined_mae = (mae+neg_mae)/(1+1)  # for validation

            return mse, combined_mae, neg_mse, ndcg, score, label, neg_prediction, neg_probs
...
'''def train_func(train_data, train_test_data, dev_data, test_data,
    ...
    for ids, cls in data:
        ...
        if step % 20 == 0 or (batch_size > 4096 and step % 10 ==0):
            ...
            # test
            test_MSE, test_MAE, _, _, score, label = test_func(test_data, device, model, params, ndcg_also=False)
