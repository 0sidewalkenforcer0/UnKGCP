...
from models import * #UKGE_logi_TF, UKGE_rect_TF, TransE_m1_TF, DistMult_m1_TF
...
class Tester(object):
    ...
    def get_mse(self, toprint=True, save_dir='', epoch=0, is_test=False):
        ...
        if not is_test:
            ...
            scores_mapping = 0.5 * scores + 0.5
            p_pos_mapping = np.clip(scores_mapping, 1e-6, 1 - 1e-6)
        ...
        if is_test:
            ...
            # Normalizing conformal prediction for CN15K
            ...
            # Calulate the normalized CP after mapping for CN15K
...
class UKGE_rect_Tester(Tester):
    ...
    def get_mse(self, toprint=False, save_dir='', epoch=0, is_test=False):
        def calculate_metrics(test_triples, qhat):
            h_batch = test_triples[:, 0].astype(int)
            r_batch = test_triples[:, 1].astype(int)
            t_batch = test_triples[:, 2].astype(int)
        ...
        def normalizing_calculate_metrics(test_triples, qhat):
            ...
            scores = self.bound_score(self.get_score_batch(h_batch, r_batch, t_batch))
            uncertainty = np.clip(scores * (1 - scores), 1e-6, None)
            pred_intervals = [[max(0, s - qhat * u), min(1, s + qhat * u)] for s, u in zip(scores, uncertainty)]
        ...
        if not is_test:
            ...
            # nonconformal score for conformal prediction
            nonconf_scores = np.abs(errors)
            conf_level = np.ceil((len(nonconf_scores) + 1) * 0.9) / len(nonconf_scores)
            param.qhat = np.quantile(nonconf_scores, conf_level, interpolation="higher")
            param.errors = errors
        ...
        if is_test:
            test_pos_triples = self.test_triples[self.test_triples[:, 3] != 0]
            coverage_pos, sharpness_pos, test_pos_scores, test_pos_label = calculate_metrics(test_pos_triples, param.qhat)
            norm_coverage_pos, norm_sharpness_pos, _, _ = normalizing_calculate_metrics(test_pos_triples, param.normalizing_qhat)

            param.coverage_pos = coverage_pos
            param.sharpness_pos = sharpness_pos
            param.normalizing_coverage_pos = norm_coverage_pos
            param.normalizing_sharpness_pos = norm_sharpness_pos
            param.test_pos_scores = test_pos_scores
            param.test_pos_label = test_pos_label
            ...
            norm_coverage_neg, norm_sharpness_neg, _, _ = normalizing_calculate_metrics(test_neg_triples, param.normalizing_neg_qhat)

            param.coverage_neg = coverage_neg
            param.sharpness_neg = sharpness_neg
            param.normalizing_coverage_neg = norm_coverage_neg
            param.normalizing_sharpness_neg = norm_sharpness_neg
            param.test_neg_scores = test_neg_scores
            param.test_neg_label = test_neg_label

            test_triples = self.test_triples
            coverage_combined, sharpness_combined, test_combined_scores, test_combined_label = calculate_metrics(test_triples, param.qhat_combined)
            norm_coverage_combined, norm_sharpness_combined, _, _ = normalizing_calculate_metrics(test_triples, param.normalizing_qhat_combined)

            param.coverage_combined = coverage_combined
            param.sharpness_combined = sharpness_combined
            param.normalizing_coverage_combined = norm_coverage_combined
            param.normalizing_sharpness_combined = norm_sharpness_combined
            param.test_combined_scores = test_combined_scores
            param.test_combined_label = test_combined_label
