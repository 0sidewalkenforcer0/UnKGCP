...
def plot_all_seeds_combined_only(results, model_name="UKGE_logi_m2"):
    for dataset_name, seeds_data in sorted(results.items()):
        for mode in ['comb', 'pos', 'neg']:  
            ...
            fig.tight_layout(rect=[0, 0, 1, 0.97])
            plt.savefig(f"PASSLEAF_{dataset_name}_entropy_cp_{model_name}_{mode}_raw_curve.png", dpi=300)
            plt.show()
...
def main(model_path=None):
    ...
    with tf.Session() as sess:
        ...
        with open(f"results_{param.whichdata}.pkl", "wb") as f:
